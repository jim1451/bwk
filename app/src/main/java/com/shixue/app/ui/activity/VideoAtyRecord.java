...
public class VideoAtyRecord extends BaseActivity {
    ...
    VODPlayer vodPlayer;
    String vodID = "";//录播id
    private int seekNum;//播放进度
    List<ChatMsg> vodMsgList = new ArrayList<>();


    Player player;
    private boolean isShowVide = false;//设置主屏是否显示video
    SweetDialog progess;//弹窗提示
    boolean isMute;//是否可以聊天(这个是在聊天监听里的)
    private boolean isMax = true;//是否最大化显示
...
//            setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        ...
        progess = new SweetDialog(VideoAtyRecord.this, SweetDialog.PROGRESS_TYPE).setTitleText("加载中...");
        progess.show();
        //获取节数据
    ...
    private void initDataBase() {
        mysql = new Myopensqlite(VideoAtyRecord.this, "VideoMessage", null, 1);
        sql = mysql.getWritableDatabase();
    }

    public void initVod() {

        vodSite = new VodSite(VideoAtyRecord.this);
    ...
    public void getIconList() {
        ...
        //获取聊天图片
        Map<String, Drawable> iconMap = SpanResource.getBrowMap(VideoAtyRecord.this);
        //聊天图片放置于list集合中
    ...
    public void initPlayer() {
        getIconList();
        userId = Long.parseLong(APP.userInfo.getBody().getUser().getMobile()) + "";
        msgList = MySqlite.getAllMessageVideo(sql, "1");
        //设置显示参数
        player = new Player();
        changeView();
        ...
        userId = Long.parseLong(APP.userInfo.getBody().getUser().getMobile()) + "";
        //聊天消息列表显示
        mRvMessage.setLayoutManager(new LinearLayoutManager(VideoAtyRecord.this));
        Log.e("order", "initPlayer" + msgList.size());
        mReAdpt = new SingleReAdpt<VideoMessageBean>(VideoAtyRecord.this, R.layout.recycler_video_message, msgList) {
            @Override
        ...
        player.setOnChatListener(new OnChatListener() {
            ...
            public void onChatWithPerson(long l, String s, int i, String s1, String s2, int i1) {
                //私聊通知
                L("id:私聊" + l + "==用户名：" + s + "==Role：" + "==纯文本：" + s1 + "==富文本：" + s2 + "==聊天id：" + i1);
            }
            ...
            public void onChatWithPublic(long l, String s, int i, String s1, String s2, int i1) {
                ...
                userId = l + "";
                Log.e("order", "onChatWithPublic" + userId);
                //在这里要删除除了自己的所有数据
                //插入聊天数据
                if (!APP.userInfo.getBody().getUser().getMobile().equals("" + l)) {
                    MySqlite.deleteMessage(sql, "" + l);
                }
                MySqlite.insertIntoMessage(sql, l + "", s, s1, s2, i1 + "", "1");
                Message message = new Message();
                message.what = 100;
        ...
        //加入监听
        player.join(VideoAtyRecord.this, param, new OnPlayListener() {
            @Override
    ...
    public void showtitleDialog(int type, int error) {
        ...
        if (!title.equals("")) {
            new SweetDialog(VideoAtyRecord.this, SweetDialog.WARNING_TYPE)
                    .setTitleText(title)
...
//                    L.e("请求刷新适配器：" + messageBean.getName() + "----内容：" + messageBean.getChatText());
    ...
    protected void onDestroy() {
        ...
        if (player != null) {
            player.leave();//退出直播间
            player.release(VideoAtyRecord.this);//释放播放器资源
        }
        if (vodPlayer != null) {
            vodPlayer.release();
            //    VodSite.release();
    ...
    private void changeView() {
        if (isVod && vodPlayer == null || !isVod && player == null)
            return;
...
//横屏设置
            ...
            case R.id.img_max_change:
                ...
                changeView();
                break;
            case R.id.tv_min://竖屏全屏
                isMax = false;
//                setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
//竖屏设置
                changeView();
                break;
            case R.id.tv_sendMsg://发送消息
                //发送消息
                ...
                userId = player.getSelfInfo().getUserId() + "";

                MySqlite.insertIntoMessage(sql, player.getSelfInfo().getUserId() + "", player.getSelfInfo().getName(), mEditMessage.getChatText(), mEditMessage.getRichText(), player.getSelfInfo().getChatId() + "", "1");
